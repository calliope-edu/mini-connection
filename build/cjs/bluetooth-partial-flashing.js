...
exports.BluetoothPartialFlashSession = exports.BluetoothPartialFlashServiceMissingError = exports.BluetoothPartialFlashInvalidHexError = exports.BluetoothPartialFlashDalMismatchError = exports.PARTIAL_FLASH_CHARACTERISTIC_UUID = exports.PARTIAL_FLASH_SERVICE_UUID = void 0;
exports.parseMakeCodeHex = parseMakeCodeHex;
/**
...
exports.PARTIAL_FLASH_SERVICE_UUID = bluetooth_profile_js_1.profile.partialFlashing.id;
exports.PARTIAL_FLASH_CHARACTERISTIC_UUID = bluetooth_profile_js_1.profile.partialFlashing.characteristics.control.id;
 ...
 */
class BluetoothPartialFlashDalMismatchError extends Error {
    constructor() {
...
exports.BluetoothPartialFlashDalMismatchError = BluetoothPartialFlashDalMismatchError;
/**
 ...
 */
class BluetoothPartialFlashInvalidHexError extends Error {
    constructor() {
        super("Hex does not contain a MakeCode magic marker");
        this.name = "BluetoothPartialFlashInvalidHexError";
    }
}
exports.BluetoothPartialFlashInvalidHexError = BluetoothPartialFlashInvalidHexError;
/**
 ...
 */
class BluetoothPartialFlashServiceMissingError extends Error {
    constructor() {
        super("Calliope is not exposing the partial-flashing service");
        this.name = "BluetoothPartialFlashServiceMissingError";
    }
}
exports.BluetoothPartialFlashServiceMissingError = BluetoothPartialFlashServiceMissingError;
/**
 * Parse a MakeCode-compiled Intel hex, find the magic marker and slice out the
 * MakeCode region.
 */
function parseMakeCodeHex(hex) {
    const map = nrf_intel_hex_1.default.fromHex(hex);
    // Walk segments looking for the magic marker.
    for (const [segStart, bytes] of map) {
        const u8 = bytes;
        for (let i = 0; i + MAGIC_MARKER.length + 16 <= u8.length; i += 16 // markers are 16-byte aligned in MakeCode
        ) {
            let match = true;
            ...
            if (match) {
                const dalHash = u8.slice(i + MAGIC_MARKER.length, i + MAGIC_MARKER.length + 8);
                const makeCodeHash = u8.slice(i + MAGIC_MARKER.length + 8, i + MAGIC_MARKER.length + 16);
...
class BluetoothPartialFlashSession {
    constructor(server, logging) {
        ...
        Object.defineProperty(this, "pendingResponse", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        ...
        Object.defineProperty(this, "onNotify", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: (ev) => {
        ...
        this.logging = logging ?? new logging_js_1.NullLogging();
    }
    async run(hex, opts = {}) {
    ...
    async openCharacteristic() {
        let service;
        try {
            service = await withTimeout(this.server.getPrimaryService(exports.PARTIAL_FLASH_SERVICE_UUID), 4000, "getPrimaryService(partial-flashing)");
        }
        catch (err) {
        ...
        const ch = await withTimeout(service.getCharacteristic(exports.PARTIAL_FLASH_CHARACTERISTIC_UUID), 3000, "getCharacteristic(partial-flashing)");
    ...
    async switchToPairingMode(reconnect) {
        if (!reconnect) {
...
exports.BluetoothPartialFlashSession = BluetoothPartialFlashSession;
/**
...
//# sourceMappingURL=bluetooth-partial-flashing.js.map