...
export class BluetoothPartialFlashSession {
  ...
  private async openCharacteristic(): Promise<void> {
    ...
    const ch = await withTimeout(
      service.getCharacteristic(PARTIAL_FLASH_CHARACTERISTIC_UUID),
      3000,
  ...
  private async writeNoNotify(payload: Uint8Array): Promise<void> {
    ...
    if (this.characteristic.writeValueWithoutResponse) {
      await withTimeout(
        this.characteristic.writeValueWithoutResponse(payload),
        3000,
        "writeValueWithoutResponse",
      );
    } else {
      await withTimeout(
        this.characteristic.writeValue(payload),
        3000,
        "writeValue",
      );
    }
  ...
  private async switchToPairingMode(
    ...
    if (!disconnected) {
      throw new Error(
        "Calliope did not switch to pairing mode (no disconnect after 3 attempts)",
      );
    }
